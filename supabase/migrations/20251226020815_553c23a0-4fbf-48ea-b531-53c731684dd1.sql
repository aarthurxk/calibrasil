-- Fix the functions with correct search_path that includes extensions schema
...
DECLARE
  ...
  hashed_token text;
BEGIN
  -- Generate a random token using multiple UUIDs for entropy
  raw_token := replace(gen_random_uuid()::text || gen_random_uuid()::text, '-', '');
  
  -- Hash it for storage using sha256
  hashed_token := encode(sha256(raw_token::bytea), 'hex');
  
  -- Insert or update (upsert)
  INSERT INTO public.order_confirm_tokens (order_id, token_hash, expires_at)
...
-- Update the validation function with correct search_path
...
DECLARE
  ...
  hashed_input text;
BEGIN
  -- Hash the input token using sha256
  hashed_input := encode(sha256(p_token::bytea), 'hex');
  
  -- Find the token
  SELECT * INTO token_record
  FROM public.order_confirm_tokens
  ...
  -- Mark as used
  UPDATE public.order_confirm_tokens
  SET used_at = now()
  WHERE order_id = p_order_id;
